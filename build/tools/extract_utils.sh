...
function setup_vendor() {
    ...
    export KCUF_ROOT="$3"
    if [ ! -d "$KCUF_ROOT" ]; then
        echo "\$KCUF_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$KCUF_ROOT/$OUTDIR" ]; then
        mkdir -p "$KCUF_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$KCUF_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$KCUF_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$KCUF_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$KCUF_ROOT"/vendor/kcuf/build/tools/smali/baksmali.jar
        export SMALIJAR="$KCUF_ROOT"/vendor/kcuf/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$KCUF_ROOT"/vendor/kcuf/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$KCUF_ROOT"/vendor/kcuf/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$KCUF_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$KCUF_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$KCUF_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$KCUF_ROOT"/vendor/kcuf/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$KCUF_ROOT"/"$OUTDIR"/radio

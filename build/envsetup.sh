function __print_kcuf_functions_help() {
cat <<EOF
Additional KCUFRom functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- kcufgerrit:   A Git wrapper that fetches/pushes patch from/to KCUFRom Gerrit Review.
- kcufrebase:   Rebase a Gerrit change and push it again.
- kcufremote:   Add git remote for KCUFRom Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for KCUFRom Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    KCUF_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/kcuf/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the KCUF model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch kcuf_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/kcuf-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.kcuf.device | grep -q "$KCUF_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $KCUF_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function kcufremote()
{
    ...
    local KCUF_USER=$(git config --get review.review.kcufos.org.username)
    if [ -z "$KCUF_USER" ]
    then
        git remote add kcuf ssh://review.kcufos.org:29418/$PFX$PROJECT
    else
        git remote add kcuf ssh://$KCUF_USER@review.kcufos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'kcuf' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/KCUFRom/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.kcuf.device | grep -q "$KCUF_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        kcufremote
        git push kcuf HEAD:refs/heads/'$1'
    fi
    '
}
...
function kcufgerrit() {
    ...
    local user=`git config --get review.review.kcufos.org.username`
...
function kcufrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "KCUFRom Gerrit Rebase Usage: "
        echo "      kcufrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.kcufos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.kcuf.device | grep -q "$KCUF_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/kcuf/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $KCUF_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/kcuf/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi

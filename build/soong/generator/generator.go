...
func init() {
	android.RegisterModuleType("kcuf_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kcufExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kcufExpandVariables(ctx, dep_file)
	...
	cmd := kcufExpandVariables(ctx, String(g.properties.Cmd))
